"""Собирает два снимка главы 2.5 из протокола record-run-structure.sh.

Запуск: bash record-run-structure.sh && python3 build-shots-structure.py
...
LOG = pathlib.Path("/tmp/python-book-structure/log.txt").read_text()
blocks = {}
...
SHOTS = [
    ("25-tree", "Раскладка файлов до и после переноса в пакет", "macOS · Python 3.12 · реальный вывод",
     term(("--", "ПОСЛЕ ГЛАВЫ 2.4 — ТРИ ФАЙЛА РЯДОМ"), ("s1-before", "ls"),
          ("--", "ПОСЛЕ ГЛАВЫ 2.5 — ПАКЕТ app С ДВУМЯ ПОДПАКЕТАМИ"), ("s2-after", "find . -type f (без .venv)"))),
    ("26-two-runs", "Форма импорта и команда запуска связаны", "один и тот же проект, два варианта импортов",
     term(("--", "ИМПОРТЫ ОТ КОРНЯ: from app.services.calculator import …"),
          ("s3-root-m", "python -m app.main"), ("s4-root-file", "python app/main.py"),
          ("--", "ИМПОРТЫ ВНУТРИ ПАКЕТА: from services.calculator import …"),
          ("s5-inside-file", "python app/main.py"), ("s6-inside-m", "python -m app.main"))),
]
...
parts = ['<!doctype html><html lang="ru"><meta charset="utf-8"><title>Структура и запуск Student Tools</title>',
         "<style>%s</style>" % CSS]
...
parts.append("</html>")
out = pathlib.Path(__file__).resolve().parent / "evidence-structure.html"
