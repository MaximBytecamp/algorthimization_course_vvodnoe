...
**Шаг 0.1.** Отдать папку `stend` по HTTP любым удобным способом. Годится Live
Server в VS Code, публикация на Vercel или команда в терминале, открытом в папке
`stend`:
...
> и без интернета не дойдут. Отданный по HTTP стенд шлёт события себе же и
> работает в классе без сети.
>
> Событие уходит запросом картинки: браузер загружает прозрачный пиксель
> `g/collect.gif`, а поля события едут в строке запроса этой картинки. Файл
> существует, поэтому сервер отвечает `200` и строка в панели Network зелёная.
> Так работали счётчики до появления `sendBeacon` — на занятии 1 мы разбирали этот
> способ под названием «отслеживающий пиксель». Настоящий `gtag.js` вместо этого
> шлёт `POST` на `google-analytics.com`; набор полей события одинаков, а разбираем
> мы именно поля.
...
**Status** — код ответа сервера, **Type** — каким способом запрос ушёл,
**Size** — вес ответа.

Кадр снят на сервере, который отвечает как настоящая аналитика: `204` и `ping`.
На учебном стенде в этих столбцах будет `200` и `gif` — событие уходит запросом
картинки-пикселя. Набор полей события от способа отправки не зависит, а разбираем
мы именно поля.
...
| Строка `collect` красная, код `404` или `405` | открыта старая версия стенда: она слала событие через `POST` на `/g/collect`, а такого адреса у статических серверов нет | обновить файлы стенда до текущей версии. Параметры при этом читаются полностью и на разбор красный цвет не влияет |
...
| На странице «Программа курса» источник `(not set)`, хотя ссылка была размечена | страницу открыли напрямую, а не переходом из меню — запоминать было нечего | начать с главной по размеченной ссылке и перейти по меню. В VS Code это частая причина: Live Server открывает **тот файл, который активен в редакторе** |
