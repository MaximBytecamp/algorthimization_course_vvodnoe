...
> и без интернета не дойдут. Локальный сервер принимает их сам, и всё работает
> в классе без сети. Строка запроса в панели Network будет красной с кодом `404` —
> это нормально: встроенный сервер Python не обслуживает адрес `/g/collect`, но
> параметры запроса читаются полностью, а нам нужны именно они.
...
|---|---|---|
| Строка `collect` красная, код `404` | локальный сервер Python не обслуживает адрес `/g/collect` | ничего: параметры на вкладке Payload читаются полностью |
...
| Порт `8000` занят | на нём уже что-то работает | запустить `python3 -m http.server 8001` и открыть тот же порт в браузере |
