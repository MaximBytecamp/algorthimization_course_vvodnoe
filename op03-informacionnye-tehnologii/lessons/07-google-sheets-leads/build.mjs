...
const breakdown=[
 ...
 ['js/script.js · форма на сайте',[
  ...
  ['Отправка без preventDefault',"// preventDefault() не вызываем: браузер отправляет POST сам.",
   'В теме 6 обработчик отменял отправку, потому что форма ничего никуда не отправляла. Теперь отмены нет: браузер сам выполняет POST по адресу из action. Ответ приёмника открывается в скрытом iframe, поэтому страница остаётся на месте и посетитель видит сообщение под формой.']]],
