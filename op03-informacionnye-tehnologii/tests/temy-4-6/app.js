/* Тест ОП.03 по темам 4, 5, 6. Одна попытка, 45 минут, разбор после отправки.

   Состояние живёт в localStorage этого браузера:
     op03-test-t46:state  — идёт попытка (ФИО, порядок, ответы, дедлайн, уходы)
     op03-test-t46:result — попытка завершена, оценка и ответы зафиксированы
   ...
   В заданиях есть схемы и снимки экрана; снимок увеличивается нажатием. */
...
function codeBlock(q, cls = '') {
  return `<figure class="q-code ${cls}"><figcaption><span>${esc(q.file || 'фрагмент кода')}</span><span>${esc(q.lang || '')}</span></figcaption><pre>${
    q.code.split('\n').map((line, i) => `<span class="ln" data-l="${i}">${esc(line) || ' '}</span>`).join('')}</pre></figure>`;
}

/* Схема рисуется на месте, снимок экрана открывается крупно по нажатию. */
function imageBlock(img) {
  if (!img) return '';
  if (img.svg) return `<figure class="q-img">${img.svg}<figcaption>${esc(img.caption)}</figcaption></figure>`;
  return `<figure class="q-img shot"><button type="button" class="zoom-btn" data-zoom="${esc(img.src)}" data-cap="${esc(img.caption)}">
      <img src="${esc(img.src)}" alt="${esc(img.caption)}" loading="lazy" draggable="false">
    </button><figcaption>${esc(img.caption)} · нажмите, чтобы увеличить</figcaption></figure>`;
}

function questionHead(q, pos, extra = '') {
  return `<div class="q-head">
      <span class="q-num">ЗАДАНИЕ ${pos + 1} ИЗ ${QUIZ.questions.length}</span>
      <span class="q-kind">${KIND[q.type]}</span>${extra}
...
function renderQuestions() {
  ...
  state.order.forEach((qi, pos) => {
    ...
    card.innerHTML = questionHead(q, pos)
      + imageBlock(q.image)
      + (q.code && q.type !== 'line' ? codeBlock(q) : '')
...
function bodyLine(q, body) {
  body.innerHTML = codeBlock(q, 'lines');
  const sel = state.answers[q.id] ? state.answers[q.id][0] : null;
  body.querySelectorAll('.ln').forEach(ln => {
    const l = Number(ln.dataset.l);
    ln.classList.toggle('sel', l === sel);
    ln.addEventListener('click', () => { if (commit(q, [l])) renderBody(q); });
  });
}
  ...
  switch (q.type) {
    ...
    case 'line': {
      ...
      const lines = q.code.split('\n').map((line, i) => {
        const cls = i === key[0] ? 'right' : i === my ? 'wrong' : '';
        return `<span class="ln ${cls}">${esc(line) || ' '}</span>`;
      }).join('');
      const note = my === null ? `Вы не выбрали строку.` : my === key[0] ? `Вы выбрали строку ${my + 1} — верно.` : `Вы выбрали строку ${my + 1}, верная — строка ${key[0] + 1}.`;
      return `<figure class="q-code lines-rv"><figcaption><span>зелёная — верно · красная — ваш выбор</span></figcaption><pre>${lines}</pre></figure><p>${note}</p>`;
    }
    case 'number': {
...
document.addEventListener('click', e => {
  ...
  if (btn) {
    const box = $('zoom');
    box.innerHTML = `<img src="${esc(btn.dataset.zoom)}" alt="${esc(btn.dataset.cap)}">
      <p>${esc(btn.dataset.cap)}<span>нажмите, чтобы закрыть</span></p>`;
    box.classList.remove('hidden');
    return;
  }
  if (e.target.closest('#zoom')) closeZoom();
});
