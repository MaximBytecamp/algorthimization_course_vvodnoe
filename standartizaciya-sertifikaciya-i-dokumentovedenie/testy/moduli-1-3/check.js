...
const KIND_SHORT = { single: 'выбор', multi: 'несколько', order: 'порядок', slots: 'подстановка', sort: 'группы', line: 'строка', number: 'число', odd: 'лишнее', path: 'адрес', docs: 'документы' };

async function verify(code) {
  const parts = code.trim().split('.');
  if (parts.length !== 3 || parts[0] !== QUIZ.prefix) return { fatal: `Это не код результата теста по модулям 1–3 (ожидается ${QUIZ.prefix}.…).` };
