/* Тест по дисциплине «Стандартизация, сертификация и техническое документоведение», модули 1–3.
   Одна попытка, время из QUIZ.minutes, разбор после отправки.
   ...
   Виды заданий: single, multi, order, slots, sort, line, number, odd, path, docs.
   У line с полем many можно отметить несколько строк; ответ — номера по возрастанию.
   sort — разложить карточки по группам; odd — убрать лишнее из списка в корзину;
   path — собрать адрес из сегментов; ответ — номера сегментов по порядку.
   docs — сравнить два документа Markdown: каждый показан целиком и скачивается
   кнопкой как файл .md; ответ — номер выбранного документа. */
...
function fileLabel(text) { return text; }
...
/* сравнение документов: два файла Markdown рядом, каждый скачивается как .md */
...
function docPane(q, d, i, chosen, review = '') {
  return `<div class="doc ${review || (chosen === i ? 'picked' : '')}" data-d="${i}">
    <div class="doc-head">
      <span class="doc-name">${esc(d.name)}</span>
      <button type="button" class="dl" data-dl="${i}">Скачать .md</button>
    </div>
    <pre class="doc-md">${esc(d.md)}</pre>
  </div>`;
}

function bodyDocs(q, body) {
  const cur = state.answers[q.id] || [];
  const chosen = cur.length ? cur[0] : null;
  body.innerHTML = `<div class="docs">${q.docs.map((d, i) => docPane(q, d, i, chosen)).join('')}</div>
    <div class="docs-pick">${q.docs.map((d, i) => `
      <label class="opt ${chosen === i ? 'picked' : ''}">
        <input type="radio" name="${q.id}" value="${i}" ${chosen === i ? 'checked' : ''}>
        <span>Выбираю <b>${esc(d.name)}</b></span>
      </label>`).join('')}</div>`;

  body.querySelectorAll('.dl').forEach(btn => btn.addEventListener('click', () => {
    const i = Number(btn.dataset.dl);
    download(q.docs[i].name, q.docs[i].md);
    btn.textContent = 'Файл сохранён';
    setTimeout(() => { btn.textContent = 'Скачать .md'; }, 1800);
  }));
  body.querySelectorAll('.docs-pick input').forEach(input => input.addEventListener('change', () => {
    if (commit(q, [Number(input.value)])) renderBody(q);
  }));
}
...
function reviewBody(q, a, key, notes) {
  ...
  switch (q.type) {
    ...
    case 'docs': {
      ...
      const panes = q.docs.map((d, i) => {
        const right = key.includes(i);
        const cls = right ? 'right' : (mine === i ? 'wrong' : '');
        const tag = right && mine === i ? 'ваш выбор · верно' : right ? 'верный документ' : mine === i ? 'ваш выбор · неверно' : 'с нарушениями';
        return `<div class="doc rv-doc ${cls}">
          <div class="doc-head"><span class="doc-name">${esc(d.name)}</span><span class="tag">${tag}</span></div>
          ${notes && notes[i] ? `<p class="note">${notes[i]}</p>` : ''}
          <pre class="doc-md">${esc(d.md)}</pre>
        </div>`;
      }).join('');
      return `<div class="docs">${panes}</div>${mine === null ? `<p>${none}</p>` : ''}`;
    }
    case 'odd': {
      const mine = a || [];
