...
from kit import ROOT, FIG, p, ul, table, plural  # noqa: E402
...
<html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#302330"><meta name="description" content="{escape(title)}. Справочник Макарова М. Н.: объяснения, измерения на стенде, схемы и снимки экрана."><title>{escape(title)} · Наблюдаемость · Макаров М. Н.</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="{FONT}" rel="stylesheet"><link rel="stylesheet" href="{prefix}styles.css"><script src="{prefix}assets/experiment-data.js" defer></script><script src="{prefix}book.js" defer></script></head><body>
...
def render_chapter(i, c):
    body = (f'<header class="chapter-head"><span class="chapter-num">1.{i}</span><h1>{c["title"]}</h1>'
            f'<p class="lead">{c["lead"]}</p><div class="quote"><span class="eyebrow">Главное в главе</span><p>{c["epigraph"]}</p></div></header>')
    ...
    parts = body.split(FIG)
    body = parts[0] + ''.join(f'1.{i}.{k}' + part for k, part in enumerate(parts[1:], 1))
...
body = f'''<header class="cover"><div><span class="eyebrow">Справочник · Python и работающий сервис</span><h1>Метрики<br>и наблюдаемость</h1><p class="cover-sub">От медленного запроса<br>к объяснённому инциденту</p><p>Как узнать, что происходит с приложением после запуска на сервере: что измерять, какие события записывать и как найти этап, на котором запрос теряет время. В главах — код, настоящие измерения и снимки стенда, который можно запустить у себя.</p><p class="author-line">Макаров Максим Николаевич</p><a class="primary" href="temy/{CH[0]["slug"]}.html">Начать первую тему →</a></div><aside class="cover-sheet"><span class="eyebrow">Один сервис / три сигнала</span><div class="cover-series"><svg viewBox="0 0 280 85" role="img" aria-label="Условный график: рост длительности запросов"><path d="M0 65H280M0 35H280" class="cover-grid"/><path d="M0 65L30 64L55 62L80 64L110 60L135 62L160 18L180 25L200 17L225 23L250 20L280 15" fill="none" stroke="currentColor" stroke-width="3"/></svg><b>МЕТРИКИ</b><p>Изменение видно в истории.</p></div><div class="cover-log"><b>ЛОГИ</b><code>event=request_completed<br>status=500 request_id=1cf09e6f…</code></div><div class="cover-trace"><b>ТРАССЫ</b><span style="width:100%">GET /products</span><span style="width:78%;margin-left:12%">dependency.wait</span></div><small>Условная схема, значения придуманы для обложки.</small></aside></header>
<div class="cover-meta"><span>Тема 1 · {len(CH)} {plural(len(CH), 'глава', 'главы', 'глав')}</span><span>{sections_total} {plural(sections_total, 'раздел', 'раздела', 'разделов')}</span><span>Снимки работающего стенда</span><span>Запускаемый Python-проект</span></div>
<section><h2><span class="sec">I</span>Как устроен справочник</h2><p>Первая тема посвящена мониторингу, наблюдаемости и телеметрии. Мы начинаем с небольшого сервиса на FastAPI и разбираем, что разработчик теряет после его развёртывания. Затем по отдельности разбираем метрики, логи и трассы, проводим по ним расследование и смотрим, как данные собирают Prometheus и Grafana.</p><p>Каждая глава построена одинаково: объяснение по разделам, частые ошибки, вопросы для самопроверки с ответами и шпаргалка. Все числа и снимки получены на стенде справочника; его архив можно скачать и повторить опыт. Следующие модули показаны в содержании как план.</p><div class="reading-path"><b>Ситуация</b><span>→</span><b>Механизм</b><span>→</span><b>Измерения</b><span>→</span><b>Проверка вывода</b></div></section>
